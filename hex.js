...
frame.innerHTML = `<img src="${this.src}" width="100%" style="max-width: 450px">`; 
...
scriptContent = scriptContent.replace(/\$(\w+)/g, '$<imp color=#16ABCC>$1</imp>');
...
text = text.replace(/\/\/\s*([^\n]*)/g, '<imp style="opacity: 0.5" color=white>//$1</imp>');
...
popcan.innerHTML = `<in style="cursor: pointer" onclick="hexPop('`+poprole+`')">&times;</in>`;
